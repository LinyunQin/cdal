...
from model.faster_rcnn.DA import _ImageDAencoder
from model.faster_rcnn.DA import _DAencoder
import time
import pdb
from model.utils.net_utils import _smooth_l1_loss, _crop_pool_layer, _affine_grid_gen, _affine_theta
from model.faster_rcnn.kmeans import *
...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic):
        ...
        self.image_encoder = _ImageDAencoder(self.dout_base_model)
        self.instance_encoder = _DAencoder()
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes,
        ...
        if self.training:
            ...
            # labeled attack loss
            mask = torch.ones_like(cls_score).scatter_(1, rois_label.unsqueeze(1), 0).cuda()
            attack_rois_label = torch.max(cls_score * mask, dim=1)[1]
            attack_rois_label = torch.where(rois_label > 0, attack_rois_label, rois_label)
            attack_RCNN_loss_cls = F.cross_entropy(cls_score, attack_rois_label)
        ...
        if self.training and not self.class_agnostic:
            ...
            tgt_bbox_pred_select = torch.gather(tgt_bbox_pred_view, 1, tgt_rois_label.view(rois_label.size(0), 1, 1).expand(tgt_rois_label.size(0), 1, 4))
        ...
        if self.training:
            ...
            # labeled attack loss
            mask = torch.ones_like(tgt_cls_score).scatter_(1, tgt_rois_label.unsqueeze(1), 0).cuda()
            tgt_attack_rois_label = torch.max(tgt_cls_score * mask, dim=1)[1]
            tgt_attack_rois_label = torch.where(tgt_rois_label > 0, tgt_attack_rois_label, tgt_rois_label)
            tgt_attack_RCNN_loss_cls = F.cross_entropy(tgt_cls_score, tgt_attack_rois_label)

            # bounding box regression L1 loss
            tgt_RCNN_loss_bbox = _smooth_l1_loss(tgt_bbox_pred, tgt_rois_target, tgt_rois_inside_ws, tgt_rois_outside_ws)
            base_feat = self.image_encoder(base_feat)
            tgt_base_feat = self.image_encoder(tgt_base_feat)
            all_feat  = torch.cat([base_feat, tgt_base_feat]).contiguous()

            all_feat = (all_feat - torch.min(all_feat, 1)[0].unsqueeze(1).repeat(1, 128)) / ((
                        torch.max(all_feat, 1)[0] - torch.min(all_feat, 1)[0]).unsqueeze(1).repeat(1, 128)+1e-4)
            sse_ima = kmeans(all_feat)
            sse_loss_image = sse_ima/100

            all_pooled_feat = torch.cat([pooled_feat, tgt_pooled_feat]).contiguous()
            all_pooled_feat = self.instance_encoder(all_pooled_feat)
            all_pooled_feat = (all_pooled_feat - torch.min(all_pooled_feat, 1)[0].unsqueeze(1).repeat(1, 1024)) / ((
                        torch.max(all_pooled_feat, 1)[0] - torch.min(all_pooled_feat, 1)[0]).unsqueeze(1).repeat(1, 1024)+1e-4)
            sse_ins = kmeans(all_pooled_feat)
            sse_loss_ins = sse_ins/100
            attack_RCNN_loss_cls += tgt_attack_RCNN_loss_cls
        return rois, cls_prob, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, \
               tgt_rpn_loss_cls, tgt_rpn_loss_bbox,tgt_RCNN_loss_cls, tgt_RCNN_loss_bbox,sse_loss_image,sse_loss_ins, attack_RCNN_loss_cls
