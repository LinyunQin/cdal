...
from model.faster_rcnn.vgg16_cluster_uca import vgg16
from model.faster_rcnn.resnet import resnet
from FGSM import fgsm_attack as fgsm
...
def parse_args():
  ...
  parser.add_argument('--save_dir', dest='save_dir',
                      help='directory to save models', default="models/da_fasterrcnn",
                      type=str)
  ...
  parser.add_argument('--testpart', dest='testpart',
                      help='test_s or test_t or test_all', default="bdd100k",
                      type=str)
...
if __name__ == '__main__':
  ...
  if args.dataset == "pascal_voc":
      ...
      print('loading our dataset...........')
      args.s1_imdb_name = "unioncityscape"+"_s1_2007_train"+args.testpart
      args.s2_imdb_name = "fogycityscape" + "_s1_2007_train" + args.testpart
      args.s3_imdb_name = "bdd100k" + "_s1_2007_train" + args.testpart
      args.set_cfgs = ['ANCHOR_SCALES', '[4,8,16,32]', 'ANCHOR_RATIOS', '[0.5,1,2]', 'MAX_NUM_GT_BOXES', '50']
  ...
  if args.testpart=='fogycityscape':
      s_imdb, s_roidb, s_ratio_list, s_ratio_index = combined_roidb(args.s1_imdb_name)
      s_train_size = len(s_roidb)  # add flipped         image_index*2
      print('{:d} roidb entries'.format(len(s_roidb)))
      t_imdb, t_roidb, t_ratio_list, t_ratio_index = combined_roidb(args.s3_imdb_name)
      t_train_size = len(t_roidb)  # add flipped         image_index*2
      print('{:d} roidb entries'.format(len(t_roidb)))
  elif args.testpart=='unioncityscape':
      s_imdb, s_roidb, s_ratio_list, s_ratio_index = combined_roidb(args.s2_imdb_name)
      s_train_size = len(s_roidb)  # add flipped         image_index*2
      print('{:d} roidb entries'.format(len(s_roidb)))
      t_imdb, t_roidb, t_ratio_list, t_ratio_index = combined_roidb(args.s3_imdb_name)
      t_train_size = len(t_roidb)  # add flipped         image_index*2
      print('{:d} roidb entries'.format(len(t_roidb)))
  elif args.testpart=='bdd100k':
      s_imdb, s_roidb, s_ratio_list, s_ratio_index = combined_roidb(args.s1_imdb_name)
      s_train_size = len(s_roidb)  # add flipped         image_index*2
      print('{:d} roidb entries'.format(len(s_roidb)))
      t_imdb, t_roidb, t_ratio_list, t_ratio_index = combined_roidb(args.s2_imdb_name)
      t_train_size = len(t_roidb)  # add flipped         image_index*2
      print('{:d} roidb entries'.format(len(t_roidb)))
  else:
      raise ValueError("测试集不支持")
  ...
  im_data = Variable(torch.FloatTensor(1).cuda(), requires_grad=True)
  im_info = Variable(torch.FloatTensor(1).cuda())
  num_boxes = Variable(torch.FloatTensor(1).cuda())
  gt_boxes = Variable(torch.FloatTensor(1).cuda())
  need_backprop=Variable(torch.FloatTensor(1).cuda())

  tgt_im_data = Variable(torch.FloatTensor(1).cuda(), requires_grad=True)
  tgt_im_info = Variable(torch.FloatTensor(1).cuda())
  tgt_num_boxes = Variable(torch.FloatTensor(1).cuda())
  tgt_gt_boxes = Variable(torch.FloatTensor(1).cuda())
  tgt_need_backprop=Variable(torch.FloatTensor(1).cuda())
  # ship to cuda
  ...
  for epoch in range(args.start_epoch, args.max_epochs + 1):
    ...
    for step in range(iters_per_epoch):
      ...
      rois_label,tgt_rpn_loss_cls, tgt_rpn_loss_bbox,tgt_RCNN_loss_cls, tgt_RCNN_loss_bbox,sse_loss_image,sse_loss_ins,tgt_uca_loss=\
          fasterRCNN(im_data, im_info, gt_boxes, num_boxes,
                     tgt_im_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes)

      ori_loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
           + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean()\
             +tgt_rpn_loss_cls.mean()+ tgt_rpn_loss_bbox.mean()+tgt_RCNN_loss_cls.mean()\
             + tgt_RCNN_loss_bbox.mean()+sse_loss_image.mean()+sse_loss_ins.mean()
      loss_all+=ori_loss

      fasterRCNN.zero_grad()
      tgt_uca_loss.backward(retain_graph=True)
      if args.net == "vgg16":
          clip_gradient(fasterRCNN, 10.)
      data_grad = im_data.grad.data
      perturbed_data = fgsm(im_data, data_grad, epsilon=0.001)
      tgt_data_grad = tgt_im_data.grad.data
      tgt_perturbed_data = fgsm(tgt_im_data, tgt_data_grad, epsilon=0.001)

      fasterRCNN.zero_grad()
      _, _, _, \
      rpn_loss_cls, rpn_loss_box, \
      RCNN_loss_cls, RCNN_loss_bbox, \
      _,tgt_rpn_loss_cls, tgt_rpn_loss_bbox,tgt_RCNN_loss_cls, tgt_RCNN_loss_bbox,sse_loss_image,sse_loss_ins,tgt_uca_loss=\
          fasterRCNN(perturbed_data, im_info, gt_boxes, num_boxes,
                     tgt_perturbed_data, tgt_im_info, tgt_gt_boxes, tgt_num_boxes)

      uca_loss = rpn_loss_cls.mean() + rpn_loss_box.mean() \
           + RCNN_loss_cls.mean() + RCNN_loss_bbox.mean()\
             +tgt_rpn_loss_cls.mean()+ tgt_rpn_loss_bbox.mean()+tgt_RCNN_loss_cls.mean()\
             + tgt_RCNN_loss_bbox.mean()+sse_loss_image.mean()+sse_loss_ins.mean()
      loss_all+=uca_loss
